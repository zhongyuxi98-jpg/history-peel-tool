...
<head>
    ...
    <style>
        ...
        .essay-module {{ border:1px solid #dee2e6; border-radius:10px; padding:12px 14px; background:#fff; max-width: 1000px; width: 100%; }}
...
<body>
    ...
    <script>
        ...
        function renderEditableModules(container) {{
            ...
            const html = modules.map((m) => {{
                ...
                if (m.mode === 'free') {{
                    ...
                    return `
                        <div class="essay-module" data-id="${{m.id}}" data-type="${{m.type}}">
                            <div class="module-header">
                                <span class="module-tag">${{label}}</span>
                            </div>
                            <div class="editor-box">
        ...
        function renderAIFeedback(container, reviewText) {{
            ...
            // 解析 AI 反馈，查找段落引用（如 "Body 2", "Body Paragraph 2", "Body 2" 等）
            const bodyPattern = /Body\\s+(?:Paragraph\\s+)?(\\d+)/gi;
            const matches = [...reviewText.matchAll(bodyPattern)];
            const bodyRefs = [...new Set(matches.map(m => parseInt(m[1])))];
            
            // 将反馈分段，为每个提到 Body 的段落添加 Locate 按钮
            ...
            paragraphs.forEach(para => {{
                ...
                html += `<p style="margin:0 0 12px 0; line-height:1.8;">${{para}}</p>`;
                
                if (paraBodyRefs.length > 0) {{
                    paraBodyRefs.forEach(bodyNum => {{
                        html += `<button class="locate-btn" onclick="locateBodyParagraph(${{bodyNum}})">📍 Locate Body ${{bodyNum}}</button> `;
                    }});
                }}
            }});
        ...
        function locateBodyParagraph(bodyNum) {{
            // 找到对应的 Body 模块
            const bodyModules = modules.filter(m => m.type === 'body');
            if (bodyNum < 1 || bodyNum > bodyModules.length) return;
            
            const targetModule = bodyModules[bodyNum - 1];
            const moduleEl = document.querySelector(`[data-id="${{targetModule.id}}"]`);
            
            if (moduleEl) {{
                // 移除之前的高亮
                document.querySelectorAll('.essay-module').forEach(el => el.classList.remove('highlight'));
                
                // 添加高亮
                moduleEl.classList.add('highlight');
                moduleEl.scrollIntoView({{ behavior: 'smooth', block: 'center' }});
                
                // 3 秒后移除高亮
                setTimeout(() => {{
                    moduleEl.classList.remove('highlight');
                }}, 3000);
            }}
        }}
